...
const IDler = {
  botID: "911682099315093524",
  botSecret: "64K4xGKNIER417NLduAv5DQZkJk6PXgj",
  botCallbackURL: "https://yuka-code.cf/callback",
  sunucuID: "880140180898598973",
  sunucuDavet: "https://discord.gg/byUZW4RMS5",
  kodLogKanalı: "897484890222780505",
  sahipRolü: "884154639581323314",
  adminRolü: "911966601815916574",
  kodPaylaşımcıRolü: "897487717238194247",
  boosterRolü: "912734358119546921",
  kodPaylaşamayacakRoller: ["884154662914228305", "884154662914228305"],
  sablonlar: "912020164852207666",
  hazırAltyapılarRolü: "884154660880015450",
  hazırSistemlerRolü: "884154660880015450",
  sistemlerrolü: "884154660880015450",
  elmasKodlarRolü: "912734358119546921", //js+
  altınKodlarRolü: "884154659965636618", //html
  normalKodlarRolü: "884154657574891560" //JavaScript
};
...
app.get("/sablon/:id", (req, res) => {
    ...
    return res.redirect(
      url.format({
        ...
        query: {
          statuscode: 137,
          message:
            "Şablonları Görebilmek İçin Discord Sunucumuza Katılmanız | Siteye Giriş Yapmanız Gerekmektedir."
        }
      })
    );
  var id = req.params.id;
  ...
  if (code) {
    ...
    if (
      member &&
      (member.roles.cache.has(IDler.sablonlar) ||
        member.roles.cache.has(IDler.boosterRolü) ||
        member.roles.cache.has(IDler.sahipRolü) ||
        member.roles.cache.has(IDler.kodPaylaşımcıRolü) ||
        member.roles.cache.has(IDler.adminRolü))
      ...
      res.redirect(
        url.format({
          ...
          query: {
            statuscode: 501,
            message: "Bu şablonu görmek için gerekli yetkiniz yok."
          }
        })
      );
    }
...
app.post("/paylasim", (req, res) => {
  ...
  client.channels.cache.get(IDler.kodLogKanalı).send(
    new Discord.MessageEmbed()
      ...
      .addField(
        "Kod Sayfası",
        `[Tıkla!](https://yuka-code.cf/${obj.kod_rank}/${obj.id})`
      )
  );
...
client.on("ready", () => {
  const listener = app.listen(process.env.PORT, function() {
    client.user.setActivity(`Yuka Code`, { type: "WATCHING" });
